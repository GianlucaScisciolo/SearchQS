...
class ResultDynamicAnalysisDAOImpl(DAOImpl, IResultDynamicAnalysisDAO):
    ...
    CREATE = """
INSERT INTO result_dynamic_analysis (name_q_circuit, number_q_circuit, matrix, result, id_result) 
VALUES (%s, %s, %s, %s, %s);
    """
    ...
    READ_BY_ID_RESULT = """
SELECT 
    id, name_q_circuit, number_q_circuit, matrix, result, id_result 
FROM 
...
WHERE 
    ...
    def create(self, rda):
        is_created = self.execute_action(self.CREATE, (rda.name_q_circuit, rda.number_q_circuit, rda.matrix, rda.result, rda.id_result), 1)
        if is_created:
            return is_created
        raise Exception
    ...
    def read_by_id_result(self, id_r):
        ...
        for index in range(0, len(results_dynamic_analysis)):
            ...
            (rda.id, rda.name_q_circuit, rda.number_q_circuit, 
