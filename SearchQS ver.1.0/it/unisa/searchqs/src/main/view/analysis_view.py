...
#import requests
#from model.entity.q_gate import QGate
...
def display_analyses_transpilation_selected():
    ...
    if response['success']:
        ...
        #response['name_transpilation'] = attributes['name_transpilation']
    else:
        page_html = "errortemplate/error.html"
    return render_template(page_html, params=response)
    """
...
def execution_analyses():
    ...
    if response['success']:
        #transpilations = service.display_names_transpilation()
        page_html = 'analysistemplate/names_transpilation.html'
    elif response['success'] == False:
        #response['transpilations_json'] = attributes['transpilations_json']
        #response['files_json'] = attributes['files_json']
        #response['transpilations'] = json.loads(attributes['transpilations_json'])
        #response['python_files'] = json.loads(attributes['files_json'])
